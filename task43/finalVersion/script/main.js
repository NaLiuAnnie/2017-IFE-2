...
function selPriority(){
    var len = 0;
    selpri[len] = event.target.getAttribute('data-index');
    len = len + 1;
    var priorityItem = document.getElementById('priority').children;
    for(var i = 0; i < 3; i++) {
        if(priorityItem[i].children[0].style.borderColor == 'red') {
            selpri[len] = priorityItem[i].children[0].getAttribute('data-index');
            len = len + 1;
        }
    }
    event.target.style.border = '2px solid red';
    selectedPri = event.target.getAttribute('data-index');
    var allLen = currentTaskItem.length;
    document.getElementById('tasklist').innerHTML = '';
    for(var j = 0; j < allLen; j++) {
        for(var i = 0; i < len; i++) {
            if(selpri[i] == currentTaskItem[j].priority) {
                var task = createItem(currentTaskItem[j].index, currentTaskItem[j].status, currentTaskItem[j].content, currentTaskItem[j].priority);
...
function selStatus() {
    var stalen = 0;
    selsta[stalen] = event.target.getAttribute('data-index');
    stalen = stalen + 1;
    var statisItem = document.getElementById('status').children;
    for(var i = 0; i < 3; i++) {
        if(statisItem[i].children[0].style.borderColor == 'red') {
            selsta[stalen] = statisItem[i].children[0].getAttribute('data-index');
            stalen = stalen + 1;
        }
    }
    event.target.style.border = '2px solid red';
    ...
    var allLen = currentTaskItem.length;
    document.getElementById('tasklist').innerHTML = '';
    var prilen = selpri.length;
    for(var j = 0; j < allLen; j++) {
        for(var i = 0; i < stalen; i++) {
            for(var u = 0; u < prilen; u++) {
                if(selpri[i] == currentTaskItem[j].priority && selsta[u] == currentTaskItem[j].priority) {
                    var task = createItem(currentTaskItem[j].index, currentTaskItem[j].status, currentTaskItem[j].content, currentTaskItem[j].priority);
                    document.getElementById('tasklist').appendChild(task);
